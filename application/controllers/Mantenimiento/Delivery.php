...
class Delivery extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('Mantenimiento/MDelivery');
        $this->load->model('Recurso');
    }
    public function index()
    {
        $this->load->view('Mantenimiento/Delivery');
    }
    ...
    {
        $rspta = $this->MDelivery->ListarDelivery();
        $data  = array();
    ...
    public function InsertUpdateDelivery()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (empty($_POST['Delivery_idDelivery'])) {
                ...
                /* valida Delivery */
                $Validacion=$this->MDelivery->ValidacionUbigeo();

                if ($Validacion > 0) {
                    $data['Error'] = true;
                    $data['Mensaje'] .= 'Delivery para la Ubicación, ya se encuentra registrado ';
                }
                if($_POST["DeliveryDelivery"]==0){
                    $data['Error'] = true;
                ...
                if ($data['Error']) {
                    ...
                    $registro = $this->MDelivery->RegistroDelivery();
               ...
               /* valida Delivery */
                ...
                if ($data['Error']) {
                    ...
                    $registro = $this->MDelivery->UpdateDelivery();
                    if ($registro['Registro']) {
    ...
    {
        $data = $this->MDelivery->ObtenerDelivery();
        echo json_encode($data);
    }
    public function EliminarDelivery()
    {
        ...
        $delete = $this->MDelivery->EliminarDelivery();
        if ($delete['Delete']) {
